...
public class StaticWeatherAnalyze {
    ...
    {
        try {
            JSONObject details = json.getJSONArray("weather").getJSONObject(0);JSONObject main = json.getJSONObject("main");
            return details.getString("description").toUpperCase() +
                    "\n" + "Влажность: " + main.getString("humidity") + "%" +
                    "\n" + "Давление: " + main.getString("pressure") + " hPa";} catch (JSONException e)
        {e.printStackTrace();
        }
        return "NaN";
    }
