package com.example.pz7;

import android.os.Bundle;
import android.view.MenuItem;
import android.view.View;
import android.widget.ImageView;

import androidx.activity.EdgeToEdge;
import androidx.annotation.NonNull;
import androidx.appcompat.app.AppCompatActivity;
import androidx.core.graphics.Insets;
import androidx.core.view.ViewCompat;
import androidx.core.view.WindowInsetsCompat;

import com.google.android.material.bottomnavigation.BottomNavigationView;

public class MainActivity extends AppCompatActivity {

    ImageView imageMail, imageDial, imageMap, imageInfo;

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        //EdgeToEdge.enable(this);
        setContentView(R.layout.activity_main);
        //ViewCompat.setOnApplyWindowInsetsListener(findViewById(R.id.main), (v, insets) -> {
        //    Insets systemBars = insets.getInsets(WindowInsetsCompat.Type.systemBars());
        //    v.setPadding(systemBars.left, systemBars.top, systemBars.right, systemBars.bottom);
        //    return insets;
        //});
         imageMail = (ImageView) findViewById(R.id.imageMail);
        imageMap = (ImageView) findViewById(R.id.imageMap);
        imageDial = (ImageView) findViewById(R.id.imageDial);
        imageInfo = (ImageView) findViewById(R.id.imageInfo);

        BottomNavigationView bottomNavigationView = (BottomNavigationView) findViewById(R.id.bottom_navigation);
        bottomNavigationView.setOnNavigationItemSelectedListener(
                new BottomNavigationView.OnNavigationItemSelectedListener() {
                    @Override
                    public boolean onNavigationItemSelected(@NonNull MenuItem item) {

                        if (item.getItemId() == R.id.navigation_home) {
                            imageMap.setVisibility(View.VISIBLE);
                            imageDial.setVisibility(View.GONE);
                            imageMail.setVisibility(View.GONE);
                            imageInfo.setVisibility(View.GONE);
                            return true;
                        } else if (item.getItemId() == R.id.navigation_dashboard) {
                            imageMap.setVisibility(View.GONE);
                            imageDial.setVisibility(View.VISIBLE);
                            imageMail.setVisibility(View.GONE);
                            imageInfo.setVisibility(View.GONE);
                            return true;
                        } else if (item.getItemId() == R.id.navigation_notifications) {
                            imageMap.setVisibility(View.GONE);
                            imageDial.setVisibility(View.GONE);
                            imageMail.setVisibility(View.VISIBLE);
                            imageInfo.setVisibility(View.GONE);
                            return true;
                        } else if (item.getItemId() == R.id.navigation_info) {
                            imageMap.setVisibility(View.GONE);
                            imageDial.setVisibility(View.GONE);
                            imageMail.setVisibility(View.GONE);
                            imageInfo.setVisibility(View.VISIBLE);
                            return true;
                        }

                        return false;
                    }
                });

    }
}